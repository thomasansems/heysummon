...
description: HeySummon security architecture with Guard proxy, E2E encryption, Ed25519 receipts, HMAC key hashing, and IP binding.
---
...
HeySummon is built with a **security-first, content-blind relay** architecture. The platform can route and store messages, but cannot read encrypted content. It does see metadata such as reference codes, timestamps, and request status by design.
...
| Layer | What it does |
|-------|-------------|
| **Content safety** | Built-in middleware scans all content for XSS, PII, and malicious URLs |
...
- **Malicious content check** — CI fails if content safety lets CC/SSN/BSN/XSS through
