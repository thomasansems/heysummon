...
interface StepCreateProviderProps {
  onCreated: (data: {
    providerId: string;
    providerKey: string;
    providerName: string;
...
export function StepCreateProvider({ onCreated }: StepCreateProviderProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      onCreated({ providerId, providerKey, providerName: name.trim(), channel });
    } catch (err) {
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-muted-foreground">
        A provider is the expert who answers help requests. Choose how you want to
