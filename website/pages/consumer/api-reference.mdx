...
| `requiresApproval` | ❌ | When `true`, the provider sees Approve/Deny buttons instead of a free-text reply field |
...
When you submit a request with `requiresApproval: true`, the provider receives Approve/Deny buttons in their notification channel (Telegram, Slack, or OpenClaw) instead of a free-text reply prompt.
...
// status.status === "responded" when the provider has decided
```
...
// The provider's message contains "approved" or "denied"
```

<Callout type="info">
  The `approvalDecision` field on the request is also set to `"approved"` or `"denied"` once the provider acts. Each request can only receive one decision — subsequent button presses are ignored.
</Callout>
