...
interface StepTestProviderProps {
  providerId: string;
...
export function StepTestProvider({
  providerId,
  channel,
  onSuccess,
}: StepTestProviderProps) {
  ...
  const sendTest = async () => {
    ...
    setElapsed(0);
    try {
      const res = await fetch("/api/v1/onboarding/test-provider", {
        method: "POST",
      ...
      intervalRef.current = setInterval(async () => {
        ...
        try {
          const pollRes = await fetch(
            `/api/v1/onboarding/test-provider?testId=${data.testId}&providerId=${providerId}`
          );
          if (pollRes.ok) {
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-sm text-muted-foreground">
        Send a test message to verify your provider receives notifications and can
        respond.
      </p>
        ...
        <div className="flex items-center gap-3 rounded-lg border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/20 p-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Your provider is working correctly.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/20 p-4">
            ...
            <p className="text-xs text-muted-foreground">
              The test timed out after 120 seconds. Make sure your provider channel is
