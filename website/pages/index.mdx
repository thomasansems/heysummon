...
```
AI Agent  ──POST /api/v1/help──▶  HeySummon  ──notification──▶  Human Expert
                                                              │
...
4. **Agent gets the answer** — via HTTP polling

---

## Key features

- **End-to-end encrypted** — X25519 DH + AES-256-GCM + Ed25519 signatures. The platform stores ciphertext it cannot read.
- **Simple integration** — HTTP polling for updates. No persistent connections needed.
...
- **Multi-provider** — Multiple human experts. Requests routed to available providers.
- **CLI installer** — `npx heysummon` gets you running in under 2 minutes.
- **IP Security** — Provider and client keys can be bound to specific IPs and devices.
...
- [API Reference](/reference/api) — Full endpoint reference
- [Self-Hosting](/self-hosting/docker) — Deploy with Docker
- [Provider Guide](/provider/conversations) — Responding to requests
