...
/**
 * Validate an expert key (hs_exp_...) with IP binding and device secret.
 *
 * Flow:
 *   1. Lookup expert by key
 ...
 */
export async function validateExpertKey(
  request: Request
): Promise<
  | { ok: true; expert: { id: string; userId: string; name: string } }
...
> {
  ...
  if (!expert) {
    return {
      ok: false,
      response: NextResponse.json(
        { error: "Invalid or inactive expert key" },
        { status: 401 }
      ),
    };
  }
  ...
  const existingEvents = await prisma.ipEvent.findMany({
    where: { profileId: expert.id },
  });

  if (existingEvents.length === 0) {
    // First ever request — bind this IP as allowed
    await prisma.ipEvent.create({
      data: { profileId: expert.id, ip: clientIp, status: "allowed" },
    });
  } else {
    ...
    if (matchingEvent) {
      if (matchingEvent.status === "blacklisted") {
        return {
          ...
          response: NextResponse.json(
            {
              error: "IP address is blacklisted for this expert key",
      ...
      if (matchingEvent.status === "pending") {
        ...
        return {
          ...
          response: NextResponse.json(
            {
              error: "IP address not authorized for this expert key",
      ...
      await prisma.ipEvent.upsert({
        where: {
          profileId_ip: { profileId: expert.id, ip: clientIp },
        },
        create: {
          profileId: expert.id,
          ip: clientIp,
  ...
  if (expert.deviceSecret) {
    const deviceToken = request.headers.get("x-device-token");
    if (!deviceToken || hashDeviceToken(deviceToken) !== expert.deviceSecret) {
      return {
        ok: false,
        response: NextResponse.json(
          {
            error: "Invalid or missing device token",
            hint: "This expert key requires a device token. Include x-device-token header with the secret generated during setup.",
          },
          { status: 403 }
        ),
      };
  ...
  if (expert.machineId) {
    if (!machineId || machineId !== expert.machineId) {
      return {
        ok: false,
        response: NextResponse.json(
          {
            error: "Machine fingerprint mismatch. This expert key is bound to another device.",
  ...
  return {
    ok: true,
    expert: { id: expert.id, userId: expert.userId, name: expert.name },
  };
}
