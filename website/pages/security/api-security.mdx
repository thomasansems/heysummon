...
| Poll pending events | Client or provider key |
| Send provider response | Provider key (`hs_prov_*`) |
| Close a request | Provider key — only assigned provider |
| Key management | Dashboard session only |

**IDOR protection** — Clients can only access their own requests. Providers can only respond to requests assigned to their profile.

## Rate limiting

Rate limiting is applied by the Platform's edge middleware (not Guard). Two layers:
