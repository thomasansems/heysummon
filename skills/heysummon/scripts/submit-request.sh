#!/bin/bash
# HeySummon — Submit a help request (OpenClaw-style: non-blocking + auto-watcher)
...
if command -v pm2 &>/dev/null; then
  ...
  if [ "$PM2_STATUS" != "online" ]; then
    ...
    bash "$SCRIPT_DIR/openclaw-setup.sh" 2>/dev/null || true
  else
