...
const mockUpsert = vi.mocked(prisma.expertMetrics.upsert);

describe("expert-metrics", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockUpsert.mockResolvedValue({} as never);
  });

  it("calculates correct metrics for an expert with history", async () => {
    // responded count
    mockCount
    ...
    await recalculateMetrics("exp-1");

    expect(mockUpsert).toHaveBeenCalledWith({
      where: { expertId: "exp-1" },
      create: {
        expertId: "exp-1",
        avgRating: 4.5,
  ...
  it("handles expert with no requests (empty state)", async () => {
    mockCount
    ...
    await recalculateMetrics("exp-empty");
  ...
  it("calculates 100% reliability when no expired requests", async () => {
    ...
    await recalculateMetrics("exp-reliable");
  ...
  it("calculates 0% reliability when all requests expired", async () => {
    ...
    await recalculateMetrics("exp-unreliable");
  ...
  it("handles database errors gracefully without throwing", async () => {
    ...
    await expect(recalculateMetrics("exp-error")).resolves.toBeUndefined();
