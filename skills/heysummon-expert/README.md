...
When an AI agent gets stuck, it can send a help request through HeySummon. As an **expert**, you receive these requests as notifications and can respond directly from your messaging app (e.g. Telegram).

## Architecture

```
AI Agent → HeySummon Platform → HTTP Polling → Expert Watcher → Notification (Telegram/etc)
...
| `HEYSUMMON_API_KEY` | Yes | Your expert key (`hs_exp_...`) from the dashboard |
| `HEYSUMMON_NOTIFY_TARGET` | Yes | Chat ID for notifications (e.g. Telegram chat ID) |
...
- **Expert key validation** — scripts reject non-expert keys
...
| `experts.json` | Cached expert names (auto-populated) |
