#!/bin/bash
# HeySummon Expert Watcher — HTTP polling for pending events
...
SEEN_FILE="$HOME/.heysummon-expert/seen-events.txt"
EVENTS_FILE="$HOME/.heysummon-expert/events.jsonl"
...
echo "Expert watcher started (pid $$)"
...
process_event() {
      ...
      try {
        ...
        switch(type) {
          case 'new_request':
            ...
            break;
          case 'new_message':
            // Skip echo: don't notify expert about their own sent messages
            if(from === 'expert') { console.log('{}'); return; }
  ...
  if [ "$DEBUG" = "true" ]; then
    ...
    echo "[DEBUG] Message for expert: $msg" >&2
  fi
  ...
  if [[ -n "$response" ]]; then
    ...
    if [[ "$quiet" == "yes" ]]; then
      ...
      if [[ "$CURRENT_INTERVAL" != "$POLL_INTERVAL" ]]; then
        echo "Quiet hours ended — resuming normal poll (${POLL_INTERVAL}s)"
        CURRENT_INTERVAL="$POLL_INTERVAL"
      fi
