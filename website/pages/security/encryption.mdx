...
HeySummon uses an **end-to-end encrypted** hybrid encryption scheme. The server stores ciphertext it cannot read.

<Callout type="warning">
  HeySummon is a **content-blind relay**, not a zero-knowledge system. The platform can see metadata (reference codes, timestamps, request status) but cannot read encrypted message content. See [Security Overview](/security/overview) for the full visibility table.
</Callout>
...
```json
{
  ...
  "consumerSignPubKey": "MCo...hex-encoded-Ed25519-pubkey...",
  "consumerEncryptPubKey": "MCo...hex-encoded-X25519-pubkey...",
  "providerSignPubKey": "MCo...hex-encoded-Ed25519-pubkey...",
  "providerEncryptPubKey": "MCo...hex-encoded-X25519-pubkey..."
}
```

The `question` field is ciphertext — the platform cannot read it. Public keys are stored for routing only; the shared secret is derived locally and never touches the server.
