#!/bin/bash
# HeySummon Expert — Respond to a help request by request ID
...
RESULT=$(curl -s -X POST "${BASE_URL}/api/v1/message/${REQUEST_ID}" \
  ...
  -H "x-api-key: ${API_KEY}" \
  -d "$(node -e "console.log(JSON.stringify({plaintext:process.argv[1],from:'expert'}))" "$RESPONSE_TEXT" 2>/dev/null)")
