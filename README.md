...
Connect AI agents with human experts — E2E encrypted, self-hostable.
...
HeySummon is an open-source platform that lets AI agents ask humans for help when they get stuck. An agent sends an encrypted help request, a human expert answers through the dashboard, and the agent picks up the response via polling.
...
| | Feature | Description |
|---|---|---|
| 📡 | **Polling-based** | Simple, reliable HTTP polling — no WebSocket complexity |
...
- Providers poll for events every 30 seconds — simple, reliable HTTP
