...
| `UserProfile` | Provider profiles — availability, timezone, quiet hours |
| `ApiKey` | Client (`hs_live_*`) and provider (`hs_prov_*`) keys |
...
| `ChannelProvider` | Telegram/Slack/WhatsApp integrations per provider |
| `IpEvent` | IP binding events per key / profile |
| `SetupToken` | Setup link JWTs for client onboarding |
| `ProviderMetrics` | Aggregated provider stats (ratings, response times) |
| `MissedRequest` | Tracks requests missed during provider unavailability |
