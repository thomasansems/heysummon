...
function SheetOverlay({
  ...
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        "fixed inset-0 z-50 bg-black/10 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    />
  ...
  return (
    <SheetPortal>
      ...
      <DialogPrimitive.Content
        ...
        data-side={side}
        className={cn(
          "fixed z-50 flex flex-col gap-4 bg-background text-sm shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
          side === "right" &&
