...
#   2. Run init --yes --from-source . (uses local repo instead of GitHub download)
#   3. Wait for health at port 3435
#   4. Verify API responses
#   5. Test tunnel detection (cloudflared if available)
...
#   NPX_FROM_SOURCE   Path to local source directory (default: current directory)
...
cleanup() {
  ...
  rm -rf ~/.heysummon 2>/dev/null || true
  echo -e "  ${DIM}Daemon stopped, data removed${NC}"
}
...
echo -e "${CYAN}From source:${NC} ${FROM_SOURCE}"
echo ""
...
section "2. NPX init --yes --from-source ${FROM_SOURCE}"

$CLI --yes --from-source "${FROM_SOURCE}" 2>&1 | tail -30
...
NOAUTH_STATUS=$(curl -sf -o /dev/null -w "%{http_code}" "${BASE_URL}/api/v1/help" 2>/dev/null || echo "000")
if [ "$NOAUTH_STATUS" = "401" ] || [ "$NOAUTH_STATUS" = "400" ] || [ "$NOAUTH_STATUS" = "000" ]; then
...
STATUS_OUT=$($CLI status 2>&1) || true
if echo "$STATUS_OUT" | grep -qi "stopped\|not running\|not initialized"; then
