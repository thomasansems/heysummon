...
Multi-line values (like the markdown summon context emitted by the dashboard install) are stored as single-quoted shell strings so `set -a; source .env; set +a` round-trips them cleanly.
